...
import { Menu, X, ArrowLeft, Clock, Calendar } from "lucide-react";
    ...
    >
      ...
      <nav className="fixed top-0 left-0 right-0 z-50 bg-[#7B1C1C]/75 backdrop-blur-lg flex items-center justify-between px-8 h-14 max-w-460 mx-auto shadow-[0_4px_20px_rgba(0,0,0,0.2)]">
        ...
        <ul className="hidden md:flex gap-7 list-none">
          {[
            ...
            <li key={item.label}>
              <Link href={item.href} className="text-white/85 text-[13px] tracking-wide hover:text-white transition-colors">
                {item.label}
              </Link>
            </li>
          ))}
      ...
      {open && (
        <div className="fixed inset-0 z-50 bg-black/30 backdrop-blur-sm" onClick={() => setOpen(false)}>
          <div className="absolute right-0 top-0 h-full w-72 bg-[#7B1C1C] p-6 flex flex-col gap-6 shadow-xl" onClick={(e) => e.stopPropagation()}>
            <button onClick={() => setOpen(false)} className="text-white self-end"><X size={24} /></button>
